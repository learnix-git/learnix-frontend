...
import { RequestTips } from "@/components/request/RequestTips";
...
export default function ClientPostPage() {
    ...
    // Trạng thái danh sách địa giới hành chính (Tỉnh/Thành, Quận/Huyện, Phường/Xã)
    ...
    // Lọc danh sách môn học theo từ khóa tìm kiếm (bỏ qua những môn đã chọn)
                ...
                <main className="flex-1 overflow-y-auto bg-slate-50 dark:bg-[#09090b] pb-24">
                    <form id="client-post-form" onSubmit={handleSubmit} className="px-6 py-10 sm:px-10 lg:px-14 xl:px-16 max-w-4xl mx-auto space-y-6">
                        ...
                        <SectionCard index={1} icon={<BookOpen className="h-4 w-4 text-primary" />} title="Thông tin chung" id="title">
                            <RequestInfo
                                form={form} errors={errors} handleUpdate={handleUpdate}
                                subjectsLoading={subjectsLoading} subjectSearch={subjectSearch}
                                setSubjectSearch={setSubjectSearch} filteredSubjects={filteredSubjects}
                            />
                        </SectionCard>
                        ...
                        <SectionCard index={3} icon={<MapPin className="h-4 w-4 text-primary" />} title="Hình thức" id="mode">
                            <RequestMode
                                form={form} errors={errors} handleUpdate={handleUpdate}
                                provinces={provinces} provincesLoading={provincesLoading}
                                provinceCode={provinceCode} handleProvinceChange={handleProvinceChange}
                                wards={wards} wardsLoading={wardsLoading} handleWardChange={handleWardChange}
                            />
                        </SectionCard>
                        ...
                        <SectionCard index={5} icon={<Clock className="h-4 w-4 text-primary" />} title="Lịch học" id="schedule">
                            <RequestTime
                                form={form} errors={errors} handleUpdate={handleUpdate}
                                handleSlotChange={handleSlotChange} toggleDay={toggleDay}
                            />
                        </SectionCard>
                        ...
                        <div className="flex flex-col-reverse sm:flex-row items-center justify-end gap-3 pt-6 border-t border-slate-200 dark:border-white/5">
                            <Button
                                ...
                                variant="outline"
                                onClick={() => router.back()}
                                className="w-full sm:w-auto rounded-2xl h-12 px-6 border border-white/50 dark:border-white/10 bg-white/60 dark:bg-white/5 backdrop-blur-md hover:bg-white/30 dark:hover:bg-white/10 transition-all font-bold text-xs tracking-widest shadow-sm"
                            >
                                Hủy bỏ
                            </Button>
                            <Button
                                ...
                                type="submit"
                                loading={submitting}
                                className="w-full sm:w-auto rounded-2xl h-12 px-8 bg-primary hover:bg-primary/90 text-white shadow-lg shadow-primary/30 font-bold text-xs tracking-widest transition-all hover:scale-[1.02]"
                            >
                                Đăng bài ngay
                            </Button>
                        </div>
